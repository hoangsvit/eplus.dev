...
ogImage: https://cdn.hashnode.com/res/hashnode/image/upload/v1763360223368/22673614-7e06-40a3-a807-80db100ec4e4.png
tags: terraform-essentials-cloud-firestore-database-gem-terraform-firestore-create, terraform-essentials-cloud-firestore-database, gem-terraform-firestore-create
...
1. Set your Project ID: `qwiklabs-gcp-04-dc5bbecfd70c`
    
    ```apache
    gcloud config set project qwiklabs-gcp-04-dc5bbecfd70c
    ```
...
4. Create a Cloud Storage bucket to store the Terraform state.
    ...
    ```apache
    gcloud storage buckets create gs://qwiklabs-gcp-04-dc5bbecfd70c-tf-state --location=us
    ```
...
1. Create a file named `main.tf` with the following content.
    ...
    terraform {
      ...
      backend "gcs" {
        bucket = "qwiklabs-gcp-04-dc5bbecfd70c-tf-state"
        prefix = "terraform/state"
      }
    }
    
    provider "google" {
      project = "qwiklabs-gcp-04-dc5bbecfd70c"
      region  = "us-central1"
    }
...
2. Create a `variables.tf` file with the following content:
    ...
    variable "project_id" {
      ...
      default     = "qwiklabs-gcp-04-dc5bbecfd70c"
    }
    
    variable "bucket_name" {
      type        = string
      description = "Bucket name for terraform state"
      default     = "qwiklabs-gcp-04-dc5bbecfd70c-tf-state"
    }
    ```
...
```apache
curl -LO raw.githubusercontent.com/ePlus-DEV/storage/refs/heads/main/labs/gem-terraform-firestore-create/lab.sh
source lab.sh
```